...
> ✅ **Terkonfirmasi resmi:** listener yang dibuat lewat `registerEvent` **otomatis di-unregister** saat script di-reload/di-disable — tidak perlu manual `unregisterEvent` di `task.bindToUnload()` demi mencegah double-listener setelah reload (kekhawatiran yang sempat muncul di bagian 8.5 soal auto-reload trigger ganda). `unregisterEvent` tetap berguna untuk cleanup manual di tengah runtime (bukan saat reload script), misal menonaktifkan fitur sementara tanpa unload seluruh script.
...
### 5.11 `Services.get(name)` — SEKARANG TERVERIFIKASI PENUH DARI DOKUMENTASI RESMI

#### `Services.get("InventoryApi")` — GUI Custom
...
const item = inventoryApi.createItem({
    ...
    lore: ["§7Baris lore 1", "§7Baris lore 2"],
    amount: 1
})

// Buat GUI
const menu = inventoryApi.constructInventory("single", "§8Judul Menu")
menu.setSlot(0, item)                 // pasang item di slot 0
menu.getSlot(0)                        // ambil item dari slot (atau null)
menu.setTitle("Judul Baru")
const salinan = menu.copy()             // WAJIB copy() per-player kalau menu dipakai bareng
menu.show(player)                       // buka untuk player
menu.hide(player)                        // tutup untuk player
menu.destroy()                           // WAJIB dipanggil saat menu tidak dipakai lagi

// Handler klik — WAJIB event.setCancelled(true) supaya item tidak bisa diambil
menu.onLeftClick(function(player, slot, event) {
    event.setCancelled(true)
    if (slot === 0) { /* aksi */ }
})
menu.onRightClick(function(player, slot, event) { event.setCancelled(true) })
menu.onItemPlaced(function(player, slot, event) { event.setCancelled(true) })
menu.onClosed(function(player, event) { menu.destroy() })  // pola umum: destroy saat ditutup
```

**Pola pemakaian benar (template + copy per player, dari dokumentasi resmi):**
```js
const inventoryApi = Services.get("InventoryApi")
const template = inventoryApi.constructInventory("single", "§8Example Menu")
template.setSlot(8, inventoryApi.createItem({ id: "minecraft:barrier", name: "§cClose" }))

addCommand("showMenu", {
    onCommand: function(player) {
        let menu = template.copy()   // WAJIB copy() supaya tiap player dapat instance sendiri
        menu.onLeftClick(function(_, slot, event) {
            event.setCancelled(true)
            if (slot === 8) { menu.hide(player); }
        })
...
```
> Ini menggantikan pola manual `Bukkit.createInventory` + serialize Base64 yang dipakai di bagian 7.3 — `InventoryApi` **lebih direkomendasikan** untuk GUI interaktif (menu, toko) karena sudah handle listener otomatis. Pola 7.3 (native Bukkit + serialize) tetap relevan khusus untuk **storage persisten lintas-restart** (virtual chest yang harus disimpan ke disk), karena `InventoryApi` tidak disebutkan native persist ke disk.

#### `Services.get("DialogApi")` — Dialog Interaktif (Butuh Paper 1.21.6+)
...
const dialog = Dialog.create(player)
    .title("&6&lKonfirmasi")
    .bodyMessage("msg", "&7Yakin mau lanjut?")
    .textInput("nama", "Masukkan nama", "")
    .rangeInput("angka", "Pilih jumlah", 5, 0, 10, 1)
    .boolInput("setuju", "Setuju?", false)
    .confirmButtons("yes", "&aYa", "no", "&cTidak")
    .onEvent(function(event) {
        if (event.type === "button" && event.id === "yes") {
            var namaInput = dialog.get("nama");   // ambil value input setelah dialog dibuka
            player.sendMessage("Kamu masukkan: " + namaInput);
        }
        dialog.close();
    })
    .show()
```
Method penting lain: `columns(n)`, `canCloseWithEscape(bool)`, `item(id, itemStack, desc)`, `baseButton(id,label)`, `urlButton(id,label,url)`, `exitButton(label)`, `destroy()` (permanen, beda dari `close()` yang bisa `show()` lagi).

#### `Services.get("PlaceholderApi")` — PlaceholderAPI Integration

⚠️ **Koreksi penting dari versi sebelumnya file ini** — signature yang benar:
```js
const PlaceholderAPI = Services.get("PlaceholderApi")

// BACA/parse placeholder jadi teks
var hasil = PlaceholderAPI.parseString(player, "Halo %player_name%!")

// DAFTAR placeholder custom — formatnya WAJIB %openjs_<prefix>_<params>%
PlaceholderAPI.registerPlaceholder("greetplr", {
    onRequest: function(player, params) {
        return "Welcome " + player.getName() + " " + params;
    }
});
// dipanggil dengan: %openjs_greetplr_to this server!%  -> params = "to this server!"

PlaceholderAPI.unregisterPlaceholder("greetplr")
```
Ini beda dari yang saya tulis di versi sebelumnya (`registerPlaceholder("undercity", "1.0", fn)`) — signature yang **benar** cuma `(prefix, {onRequest: fn})`, dan hasil placeholder-nya **selalu** berprefix `openjs_`, bukan nama bebas.

#### `Services.get("ProtocolLib")` dan `Services.get("FileManager")`
Belum sempat diverifikasi detail method-nya di sesi update ini — cek `openjs.wiki.gd/apis-and-tools/services/protocollib.md` dan `.../filemanager.md` langsung sebelum implementasi.

---

### 5.12 Aturan Wajib: Konversi JSON ke String & UUID Player sebagai String

Dua aturan disiplin ini **wajib** diikuti di semua kode yang menyentuh `DiskApi`, storage, atau logging — supaya tidak mengulang bug `[object Object]` yang pernah menyerang seluruh sistem sekaligus (lihat bagian 8.2b untuk kronologi lengkapnya).

**Aturan 1 — Objek/array WAJIB `JSON.stringify()` sebelum ditaruh ke string atau log:**
```js
// ❌ SALAH
log.info("Data: " + objekAtauArray);           // hasil: "Data: [object Object]"

// ✅ BENAR
log.info("Data: " + JSON.stringify(objekAtauArray));
```
Kalau butuh cek otomatis sebelum concat:
```js
...
**Aturan 2 — UUID Player WAJIB `.toString()` sebelum dipakai sebagai key storage:**

`player.getUniqueId()` mengembalikan objek Java `java.util.UUID`, **bukan** string JS. Kalau dipakai langsung sebagai key di `DiskApi.setVar`/`getVar` tanpa `.toString()`, hasilnya **tidak konsisten** — kadang JS otomatis convert implisit (kebetulan jalan), tapi ini **tidak boleh diandalkan** karena beda kondisi (concat vs dipakai sebagai object key vs dibandingkan dengan `===`) bisa menghasilkan representasi berbeda dari `java.util.UUID`.

```js
// ❌ RAWAN — UUID object langsung, konsistensinya tidak terjamin
var uuid = player.getUniqueId();
DiskApi.setVar(fileName, uuid, data, false);

// ✅ WAJIB — selalu .toString() dulu, konsisten di semua penggunaan
var uuid = player.getUniqueId().toString();
DiskApi.setVar(fileName, uuid, data, false);
```

**Aturan turunan penting:** karena UUID **wajib** string, kalau kamu perlu ubah balik dari string ke objek UUID (misal untuk `Bukkit.getOfflinePlayer(uuid)`), gunakan `java.util.UUID.fromString(...)`:
```js
const UUID = importClass("java.util.UUID");
var uuidString = "5b46a9d3-2f3d-4b3e-9a1e-example";  // dari storage
var uuidObject = UUID.fromString(uuidString);          // balik jadi objek UUID untuk API Bukkit
var target = Bukkit.getOfflinePlayer(uuidObject);
```

**Checklist cepat sebelum commit kode baru:**
- [ ] Setiap kali `player.getUniqueId()` dipanggil untuk dipakai sebagai key/value storage → ada `.toString()` menempel?
- [ ] Setiap kali objek/array masuk ke `log.info`/`sendMessage`/string concat → sudah `JSON.stringify()`?
- [ ] Setiap kali UUID string dari storage perlu dipakai balik ke Bukkit API → sudah `UUID.fromString()`?
